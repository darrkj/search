...
doc9 <- "this is about baseball and sports"
doc10 <- "something about movies, back to the future"


doc.list <- list(doc1, doc2, doc3, doc4, doc5, doc6, doc7, doc8, doc9, doc10)
rm(doc1, doc2, doc3, doc4, doc5, doc6, doc7, doc8, doc9, doc10)
N.docs <- length(doc.list)
names(doc.list) <- paste("doc", c(1:N.docs), sep = '')

tfidf <- function(tfr) {
  term.df <- sum(tfr[1:N.docs] > 0)
  weight <- rep(0, N.docs + 1)
  weight[tfr > 0] <- (1 + log2(tfr[tfr > 0])) * log2(N.docs / term.df)
  return(weight)
}
...
termDocMatrix <- function(...) {
  my.corpus <- Corpus(VectorSource(c(...)))
  my.corpus <- tm_map(my.corpus, tolower)
  my.corpus <- tm_map(my.corpus, removePunctuation)
  my.corpus <- tm_map(my.corpus, stemDocument)
  my.corpus <- tm_map(my.corpus, removeNumbers)
  my.corpus <- tm_map(my.corpus, stripWhitespace)
  return(as.matrix(TermDocumentMatrix(my.corpus)))
}
  
search <- function(query) {
  if (gsub(' ', '', query) == '') print ("Input Text")
  else {
    term.doc.matrix <- termDocMatrix(doc.list, query = query)
